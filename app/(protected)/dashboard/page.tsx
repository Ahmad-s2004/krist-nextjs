...
export default function DashboardPage() {
  ...
  return (
    <div className="flex-1 p-6 md:p-10 bg-neutral-50 min-h-screen font-sans">
      <div className="mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.id} className="bg-white p-6 rounded-xl border border-neutral-200/60 shadow-sm flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-neutral-400">{stat.name}</p>
                <h3 className="text-2xl font-black text-black mt-1">{stat.value}</h3>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-xl border border-neutral-200/60 shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-sm font-bold uppercase tracking-wider text-black">Recent Orders</h2>
            <button className="text-xs font-bold uppercase tracking-wider text-neutral-500 hover:text-black transition-colors">
              View All
            </button>
          </div>
              ...
              <tbody className="divide-y divide-neutral-100 text-xs">
                {recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-neutral-50/50 transition-colors">
                    ...
                    <td className="py-4 text-right font-bold text-black">{order.total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-xl border border-neutral-200/60 shadow-sm p-6 flex flex-col justify-between">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-wider text-black mb-4">Account Details</h2>
            <div className="flex items-center space-x-3 mb-6">
              <div className="h-10 w-10 rounded-full bg-black text-white flex items-center justify-center font-black text-sm">
                K
              </div>
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-black">Krist Buyer</h3>
                <p className="text-xs text-neutral-400 lowercase">krist.buyer@domain.com</p>
              </div>
            </div>
