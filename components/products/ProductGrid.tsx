  ...
  if (!loading && products.length === 0) {
    return (
      <div className="text-center py-20 border border-dashed border-gray-200">
        <p className="text-sm text-gray-400 font-medium tracking-wide">No products match your custom parameters.</p>
      </div>
    );
  }
      ...
      {loading
        ...
        : products.map((product) => (
            <div key={product.id} className="flex flex-col group space-y-3">
              <div className="relative h-[380px] bg-gray-50 border border-gray-100/60 overflow-hidden rounded-none">
                <Image
                  src={product.image}
                  alt={product.title}
                  fill
